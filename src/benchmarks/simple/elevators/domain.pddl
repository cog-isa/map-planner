...
(:action move-up-slow
  ...
  :effect (and (lift-at ?lift ?f2) (above ?f1 ?f2 ) (reachable-floor ?lift ?f2) (not (lift-at ?lift ?f1)) ))

(:action move-down-slow
  :parameters (?lift - slow-elevator ?f1 - count ?f2 - count )
  :precondition (and (lift-at ?lift ?f1) (above ?f2 ?f1 ) (reachable-floor ?lift ?f2) )
  :effect (and (lift-at ?lift ?f2) (above ?f2 ?f1 ) (reachable-floor ?lift ?f2) (not (lift-at ?lift ?f1)) ))
...
(:action board
  ...
  :effect (and  (boarded ?p ?lift) (passengers ?lift ?n2) (can-hold ?lift ?n2) (not (passenger-at ?p ?f)) (not (passengers ?lift ?n1))))

(:action leave 
  :parameters (?p - passenger ?lift - elevator ?f - count ?n1 - count ?n2 - count)
  :precondition (and  (lift-at ?lift ?f) (boarded ?p ?lift) (passengers ?lift ?n1) (next ?n2 ?n1) )
  :effect (and (passenger-at ?p ?f) (passengers ?lift ?n2) (lift-at ?lift ?f) (not (boarded ?p ?lift) (not (passengers ?lift ?n1)))))
