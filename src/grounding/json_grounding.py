...
from mapspatial.grounding.sign_task import SpTask
...
def define_situation(sit_name, cell_map, events, agent_state, signs):
    ...
    empl_image.replace('image', signs['agent'], Ag_image)
    ...
    Ag_image = agent.add_image()
    orient_image.replace('image', signs['agent'], Ag_image)
    ...
    if actuator:
        if isinstance(actuator, dict):
            if len(actuator) == 2:
                ...
                actuator_sign = signs[key]
                pred_mean = None
                for key, cm in actuator_sign.meanings.items():
                    pred_mean = cm
                    if pred_mean:
                        break
                if not pred_mean:
                    pred_image = actuator_sign.add_image()
                else:
                    pred_image = pred_mean.copy('meaning', 'image')
                ag_image = signs[ag].add_image()
                connector = sit_image.add_feature(pred_image)
                pred_image.sign.add_out_image(connector)
                conn = sit_image.add_feature(ag_image, connector.in_order)
                ag_image.sign.add_out_image(conn)
            else:
                key, item = actuator.popitem()
                pred_sign = signs[key]
                causal_signs = {signs[it] for it in itertools.chain(item['cause'], item['effect'])}
                signifs = getattr(pred_sign, 'significances')
                pr_images = getattr(pred_sign, 'images')
                image = None
                for _, si in pr_images.items():
                                    ...
                                    for sm in smaller:
                                        if sm[-1].sign == sign:
                                            if sm[-3].sign != pred_sign:
                                                replace_map[sm[-3].sign] = sign.add_image()
                                            else:
                                                replace_map[sm[-2].sign] = sign.add_image()
                                            break
                                image = si.copy('significance', 'image')
                                for key, item in replace_map.items():
                                    image.replace('image', key, item)
                                break

                if image:
                    conn = sit_image.add_feature(image)
                    image.sign.add_out_image(conn)

            sit_image = update_situation(sit_image, cell_map, signs)
        else:
            actuator = actuator.copy('meaning', 'image')
            conn = sit_image.add_feature(actuator)
            actuator.sign.add_out_image(conn)
...
def spatial_ground(problem, agent, exp_signs=None, backward = False):
